    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded bg-violet-600 px-3 py-1.5 font-mono text-xs font-semibold text-white shadow hover:bg-violet-500 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 font-mono text-xs">
        <div className="rounded-lg border border-slate-800 bg-[#0c1424] p-4 space-y-1">
          <span className="text-slate-500 text-xs uppercase block">Engine & Ruleset</span>
          <div className="text-sm font-bold text-white">v{governanceConfig.engineVersion}</div>
          <p className="text-slate-400 text-xs">Rules: {governanceConfig.rulesetVersion}</p>
        </div>

        <div className="rounded-lg border border-slate-800 bg-[#0c1424] p-4 space-y-1">
          <span className="text-slate-500 text-xs uppercase block">PQC Cryptographic Posture</span>
          <div className="text-sm font-bold text-emerald-400 flex items-center gap-1.5">
            <ShieldCheck className="size-4" />
            <span>ML-DSA-65 Valid</span>
          </div>
          <p className="text-slate-400 text-xs">NIST FIPS 204 + SHA3-256</p>
        </div>

        <div className="rounded-lg border border-slate-800 bg-[#0c1424] p-4 space-y-1">
          <span className="text-slate-500 text-xs uppercase block">Evidence Ledger Status</span>
          <div className="text-sm font-bold text-slate-200">
            {auditEntries.length + 100} Hash-Chained Blocks
          </div>
          <p className="text-emerald-400 text-xs font-semibold">0 Tamper Events Detected</p>
        </div>

        <div className="rounded-lg border border-slate-800 bg-[#0c1424] p-4 space-y-1">
          <span className="text-slate-500 text-xs uppercase block">Statutory Audit Retention</span>
          <div className="text-sm font-bold text-slate-200">7-Year Compliance</div>
          <p className="text-slate-400 text-xs">Air-Gapped Local Storage</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-slate-800 bg-[#0c1424] p-5 space-y-3 font-mono text-xs">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
          ...
          <span className="text-xs text-slate-400">
            Periodic CSE Submission <ArrowRight className="inline size-3.5 shrink-0" aria-label="to" /> Examiner Recorded Disposition
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-5 text-center pt-2">
          <div className="rounded border border-slate-800 bg-slate-900/80 p-3">
            <span className="text-violet-400 font-bold block text-xs">1. Submission</span>
            <span className="text-slate-300 text-xs mt-1 block">Raw Logs & Telemetry</span>
            <span className="text-slate-500 text-xs mt-1 block">Hashed via SHA3-256</span>
          </div>

          <div className="rounded border border-slate-800 bg-slate-900/80 p-3">
            <span className="text-violet-400 font-bold block text-xs">2. Normalization</span>
            <span className="text-slate-300 text-xs mt-1 block">Canonical Entities</span>
            <span className="text-slate-500 text-xs mt-1 block">Alerts, Cases, Assets</span>
          </div>

          <div className="rounded border border-slate-800 bg-slate-900/80 p-3">
            <span className="text-violet-400 font-bold block text-xs">3. Analytics</span>
            <span className="text-slate-300 text-xs mt-1 block">16 Workers Fusion</span>
            <span className="text-slate-500 text-xs mt-1 block">Negative Space & Gaps</span>
          </div>

          <div className="rounded border border-slate-800 bg-slate-900/80 p-3">
            <span className="text-amber-400 font-bold block text-xs">4. Human Review</span>
            <span className="text-slate-300 text-xs mt-1 block">Examiner Decision</span>
            <span className="text-slate-500 text-xs mt-1 block">Structured Rationale</span>
          </div>

          <div className="rounded border border-emerald-500/30 bg-emerald-950/20 p-3">
            <span className="text-emerald-400 font-bold block text-xs">5. TRUST-SAT</span>
            <span className="text-slate-200 text-xs mt-1 block">Immutable Chain</span>
            <span className="text-emerald-400 text-xs mt-1 block">Signed with ML-DSA-65</span>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {auditEntries.map((entry) => (
                <tr key={entry.id} className="hover:bg-slate-800/40">
                  <td className="py-2.5 px-3 font-bold text-violet-400">
                    #{entry.blockHeight}
                  </td>
                  ...
                  <td className="py-2.5 px-3">
                    ...
                    <span className="text-xs text-slate-500 block">{entry.role}</span>
                  </td>
                  <td className="py-2.5 px-3">
                    <span
                      className={`inline-block rounded px-2 py-0.5 text-xs uppercase font-bold ${
                  ...
                  <td className="py-2.5 px-3 font-sans text-xs text-slate-300 max-w-xs truncate">
                    {entry.notes}
                  </td>
                </tr>
              ))}
