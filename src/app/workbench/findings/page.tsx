    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-400">
            Evidence narratives with peer context
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 rounded-lg border border-slate-800 bg-[#0c1424] p-3 text-xs font-mono">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400 uppercase text-xs">Family:</span>
          <select
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800/80 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <Link
                  ...
                  className="font-bold text-base text-white hover:text-violet-400 transition-colors"
                >
                  {finding.title}
                </Link>
              </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-12 text-xs">
              <div className="md:col-span-8 space-y-2">
                <p className="text-slate-300 font-sans leading-relaxed">
                  <strong className="text-amber-400 font-mono text-xs uppercase block mb-0.5">
                    Observed Pattern:
                  </strong>
                  {finding.observedPattern}
                </p>
                <div className="rounded bg-slate-900/60 p-2.5 border border-slate-800/60 text-slate-400 text-xs font-sans">
                  <strong className="text-slate-300 font-mono uppercase text-xs block">
                    Why this matters:
                  </strong>
              ...
              <div className="md:col-span-4 flex flex-col justify-between border-t border-slate-800/80 pt-3 md:border-t-0 md:border-l md:pl-4 space-y-2 font-mono text-xs">
                <div className="space-y-1 text-slate-400">
                  <div>Entity: <strong className="text-white">{finding.entityName}</strong></div>
                  <div>Cohort: <span className="text-violet-300">{finding.peerContext.cohortName}</span></div>
                  <div>Evidence: <strong className="text-slate-200">{finding.evidenceRecords.length} records</strong></div>
                </div>

                <Link
                  href={`/workbench/findings/${finding.findingSlug}`}
                  className="inline-flex items-center justify-between rounded bg-violet-600 px-3 py-1.5 font-semibold text-white hover:bg-violet-500 transition-colors"
                >
